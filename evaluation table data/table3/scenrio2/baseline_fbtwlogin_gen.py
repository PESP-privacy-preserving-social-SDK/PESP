...
def getTimeList(fname):
    ...
    for four_line in logs:
        ...
        if all(["evaluation: fblogin" in l for l in four_line]):
        ...
        elif all(["twlogin" in l for l in four_line]):
...
def getPESPTimeList(fname):
    ...
    for two_line in logs:
        ...
        if all(["fblogin" in pkgname for pkgname in two_line]):
            first_l.append(delta1.microseconds / 1000)
        elif all(["twitterlogin" in pkgname for pkgname in two_line]):
...
baseline_first_l, baseline_second_l = getTimeList("baseline_fbtwlogin_log.txt")
pesp_first_l, pesp_second_l = getPESPTimeList("pesp_fbtwlogin_log.txt")
